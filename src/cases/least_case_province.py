...
df = pd.DataFrame.from_dict(least_case_provinces)
ax.barh(df.provinces,df.case)
ax.set_xlabel('Number')
for Y,X in enumerate(df.case):
    ax.annotate(X,xy=(X,Y))
ax.set_title('Least infected provinces(Per 100k population)')
least_case_canvas = FigureCanvas(fig)  # a Gtk.DrawingArea
least_case_canvas.set_size_request(700, 500)
